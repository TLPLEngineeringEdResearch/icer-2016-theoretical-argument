...
store the result back to that value. In Table 2 below, we imagine five
...
purposes). And, in each example we add a layer of specificity to the
syntax. we also propose a corresponding interpretation of how we might
apply interpret and think about a given statement.\footnote{we assume for
  ...
  addition. we also assume, crucially, that all statements are in the
...
we stress that the third column is about \emph{how we might think about}
each programming statement\emph{.} By no means am we making normative
...
an incrementer in some kind of iterative code. If we had to debug code
that employs statement 1, what we might do is exploit my knowledge of
what d is (does it hold a constant value? Is it 1?) and try inserting
intermediate print statements into the iterative code. Doing so, we can
...
easiest ways we might notice is that the acceleration seems off in the
graphics. Consequently, my debugging might call upon knowledge we have
from kinematics. we might try inserting code that draws a dot at the
object's on-screen position with each iteration, leaving a trail we can
...
first-pass test of whether my code achieves the motion we want.
...
problems in my code. If we view the assignment statement as saying
...
rather than semantically-similar. we proposed that the statements in
...
% \toprule
% \textbf{Statement } & Programming Syntax & Language & How we think about
% it\tabularnewline
% \midrule
% \endhead

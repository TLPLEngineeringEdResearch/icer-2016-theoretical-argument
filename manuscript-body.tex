...
Consider specifically lines 2 and 11. Line 2 could very well be
...
intermediate print statements to see the value of x. We might also call on our knowledge that it's being incremented by 1 to check whether the difference between any two successive printouts is 1; if it isn't, we immediately know something's wrong. In line
...
animation. Consequently, debugging might call upon knowledge we have
from kinematics. We might try inserting code that draws a dot at the
object's on-screen position with each iteration, leaving a trail of dots we can
...
first-pass test of whether the code achieves the motion we want. (If, for example, we're attempting a cubic easing animation, but the dots are all evenly spaced, then we might look first at the \texttt{displacement(t)}) function and what it's returning.
...
problems in code. If we view the assignment statement as proposing
